...
const DashboardAddProductVariations = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const existingVariations = productVariations.filter(v => v.isExisting).map(v => ({
                id: v.id,
                sizeId: v.sizeId,
                colorId: v.colorId,
                productPrice: v.productPrice,
                productVariationImage: v.productVariationImage
            }));

            const payload = { ...product, variations: [...existingVariations, ...processedNewVariations] };

            await apiClient.post(`/api/products/${productId}/product-variations`, payload, {
            ...
            <form onSubmit={handleSubmit}>
                {product && (
                    <Paper sx={{ p: 4, mb: 4 }} className="bg-white rounded-lg shadow">
                        <Typography variant="h6" className="font-semibold mb-4">Product Details</Typography>
                ...
                <Paper sx={{ p: 4 }} className="bg-white rounded-lg shadow">
                    ...
                    {productVariations.map((variation, index) => (
                        <Grid container key={index} spacing={2} sx={{ py: 2, alignItems: 'center', borderBottom: 1, borderColor: 'divider', opacity: variation.isExisting ? 0.7 : 1 }}>
                            <Grid item xs={12} md={2}>
                                <FormControl fullWidth required>
                                    <InputLabel sx={{ display: { md: 'none' } }}>Size</InputLabel>
                                    <Select value={variation.sizeId} onChange={(e) => handleVariationChange(index, 'sizeId', e.target.value)} disabled={isSubmitting || variation.isExisting} label="Size" size="small">
                            ...
                            <Grid item xs={12} md={2}>
                                <FormControl fullWidth required>
                                    ...
                                    <Select value={variation.colorId} onChange={(e) => handleVariationChange(index, 'colorId', e.target.value)} disabled={isSubmitting || variation.isExisting} label="Color" size="small">
                            ...
                            <Grid item xs={12} md={3}>
                                <TextField label="Price" type='number' fullWidth value={variation.productPrice} onChange={(e) => handleVariationChange(index, 'productPrice', e.target.value)} required inputProps={{ min: 0 }} disabled={isSubmitting || variation.isExisting} size="small"
                            ...
                            <Grid item xs={12} md={4} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                <Button component='label' size="small" variant="outlined" startIcon={<AddPhotoAlternate />} disabled={isSubmitting || variation.isExisting}>
                                    Upload
                ...
                <Box sx={{ mt: 4, display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Button type='submit' variant='contained' color='primary' size="large" disabled={isSubmitting}>
                        {isSubmitting ? 'Updating...' : 'Update Variations'}
                    </Button>
                </Box>
            </form>
        </div>
