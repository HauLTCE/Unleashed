...
import { Rating, Skeleton, Breadcrumbs, CircularProgress } from '@mui/material';
...
const ProductDetailPage = () => {
    ...
    const handleAddToCart = async () => {
        ...
        if (!selectedVariation) {
            toast.warn("Please select a color and size.", {
                position: "top-center",
                autoClose: 2000,
            });
            return;
                ...
                <div className='reviews-section mb-8'>
                    ...
                    <div className='content px-6 border-2 border-gray-300 rounded-lg p-6'>
                        {product.reviews && product.reviews.length > 0 ? (
                            <>
                                {(showAllReviews ? product.reviews : product.reviews.slice(0, 5)).map((review, index) => (
                                    <ReviewItem key={index} review={review} product={product.productId} />
                                ))}
                                {product.reviews.length > 5 && (
                                    <button onClick={() => setShowAllReviews(!showAllReviews)} className='border border-gray-300 text-black-500 font-semibold px-3 hover:scale-105 transition-transform text-base focus:outline-none mt-4 block mx-auto'>
                                        {showAllReviews ? 'Show less reviews' : `Read all reviews (${product.reviews.length - 5}+)`}
                                    </button>
                                )}
                            </>
                        ) : (
